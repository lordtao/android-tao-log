...
public class LongLog {
   ...
   private static void print(String tag, StringBuilder sb, Level level, boolean isThrowableLog) {
      ...
      for (int i = 0; i < size; i++) {
         ...
         if (cs != null) {
            ...
            print_level_log(tag, message, level);
         }
      }
   }

   private static void print_level_log(String tag, String message, Level level) {
      switch (level) {
         case VERBOSE: {
   ...
   private enum Level {
      VERBOSE, INFO, DEBUG, WARNING, ERROR, WTF, THIS_INFO
   }
