...
import ua.at.tsvetkov.util.Format.addMessage
import ua.at.tsvetkov.util.Format.addStackTrace
import ua.at.tsvetkov.util.Format.addThreadInfo
import ua.at.tsvetkov.util.Format.getFormattedMessage
import ua.at.tsvetkov.util.Format.getFormattedThrowable
import ua.at.tsvetkov.util.Format.getTag
...
object Log {
    ...
    /**
     * Is print a log string in new lines with spaces (as in AndroidStudio before 3.1). False by defaul
     */
    @Volatile
    @JvmStatic
    ...
    @JvmStatic
    fun objl(obj: Any) {
        logToAll(Level.INFO, getTag(), getFormattedMessage(Format.objl(obj), obj.javaClass.simpleName).toString())
    }
     ...
     */
    @JvmStatic
    fun isEnabled() = logCatInterceptor.isEnabled
     ...
     */
    @JvmStatic
    fun isDisabled() = logCatInterceptor.isDisabled

    @JvmStatic
    fun addInterceptor(interceptor: LogInterceptor) {
        synchronized(interceptors) { interceptors.put(interceptor.hashCode(), interceptor) }
    }
    ...
    fun removeInterceptors() {
        synchronized(interceptors) { interceptors.clear() }
    }

    @JvmStatic
    fun removeLogCatInterceptor() {
        synchronized(interceptors) { interceptors.remove(logCatInterceptor.hashCode()) }
    }

    @JvmStatic
    fun addLogCatInterceptor() {
        synchronized(interceptors) { interceptors.put(logCatInterceptor.hashCode(), logCatInterceptor) }
    }

    @JvmStatic
    fun getInterceptor(id: Int): LogInterceptor? {
        synchronized(interceptors) { return interceptors[id] }
    }

    @JvmStatic
    fun hasInterceptor(id: Int): Boolean {
        synchronized(interceptors) { return interceptors.containsKey(id) }
    }

    @JvmStatic
    fun hasInterceptor(interceptor: LogInterceptor): Boolean {
        synchronized(interceptors) { return interceptors.containsValue(interceptor) }
    }
